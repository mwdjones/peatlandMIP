...
contains
  ...
  subroutine readParams( ncid )
    ...
    !
    ! !LOCAL VARIABLES:
    character(len=32)  :: subname = 'readParams_SoilHydrologyMod'
  ...
  subroutine SurfaceRunoff (bounds, num_hydrologyc, filter_hydrologyc, &
    ...
    associate(                                                        & 
      ...
      do fc = 1, num_hydrologyc
         c = filter_hydrologyc(fc)
         !fff(c) = params_inst%fff ! MWJones edited 3/15/2023
         ...
         if ( frost_table(c) > zwt(c)) then 
            ...
            if ( frost_table(c) > zwt_perched(c)) then 
               fsat(c) = wtfact(c) * exp(-0.5_r8*params_inst%fff*zwt_perched(c))!*( frost_table(c) - zwt_perched(c))/4.0
            endif
         endif
         if (origflag == 1) then
            if (use_vichydro) then
       ...
       do fc = 1, num_hydrologyc
          ...
          if (zwt(c) < frost_table(c) .and. t_soisno(c,k_frz) <= tfrz &
             ...
             if (use_vichydro) then
                ...
                rsub_top(c)    = imped * rsub_top_max* exp(-params_inst%fff*zwt(c))
             end if
